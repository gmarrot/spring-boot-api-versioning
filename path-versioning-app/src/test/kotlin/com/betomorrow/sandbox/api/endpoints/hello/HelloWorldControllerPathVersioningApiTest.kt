...
internal class HelloWorldControllerPathVersioningApiTest : ApiTest() {
    @Test
    ...
    fun `hello world endpoint should not be accessible for invalid API version`() {
        mockMvc
            .perform(get("/invalid/hello"))
            .andExpect(status().isNotFound)

        mockMvc
            .perform(get("/vinvalid/hello"))
            .andExpect(status().isNotFound)
    }
    ...
    fun `hello endpoint with name param should not be accessible for invalid API version`() {
        mockMvc
            .perform(get("/invalid/hello/Bob"))
            .andExpect(status().isNotFound)

        mockMvc
            .perform(get("/vinvalid/hello/Bob"))
            .andExpect(status().isNotFound)
    }
